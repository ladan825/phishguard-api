...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS predictions (
        ...
        email_text TEXT NOT NULL, result TEXT NOT NULL,
        confidence REAL NOT NULL, keywords TEXT, timestamp TEXT NOT NULL)''')
    conn.commit()
    conn.close()
...
def save_prediction(email_text, result, confidence, keywords):
    conn = sqlite3.connect('phishguard.db')
    c = conn.cursor()
    c.execute('INSERT INTO predictions (email_text, result, confidence, keywords, timestamp) VALUES (?, ?, ?, ?, ?)',
        (email_text[:500], result, confidence, ', '.join(keywords), datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    conn.commit()
    conn.close()
...
def predict():
    ...
    return jsonify({'result': result, 'confidence': confidence, 'keywords': keywords, 'probability': round(float(prob_phishing) * 100, 2)})
...
def history():
    try:
        ...
        rows = c.fetchall()
        conn.close()
        return jsonify([{'id': r[0], 'text': r[1], 'result': r[2], 'confidence': r[3],
            'keywords': [k.strip() for k in r[4].split(',')] if r[4] else [], 'timestamp': r[5]} for r in rows])
    except Exception as e: return jsonify({'error': str(e)}), 500
